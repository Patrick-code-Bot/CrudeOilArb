"""
BZ-CL Grid Spread Arbitrage Strategy (NautilusTrader, Bybit single-venue)

逻辑概要：
- 在 Bybit 上订阅 BZ/USDT-LINEAR 与 CL/USDT-LINEAR 两个合约
- 实时计算价差 spread = (BZ - CL) / CL
...
class BzClGridConfig(StrategyConfig, frozen=True):
    # 交易标的（请用你实际环境里的 InstrumentId 字符串）
    # 例如："BZUSDT-LINEAR.BYBIT" 和 "CLUSDT-LINEAR.BYBIT"
    bz_instrument_id: str = "BZUSDT-LINEAR.BYBIT"
    cl_instrument_id: str = "CLUSDT-LINEAR.BYBIT"
...
class GridPositionState:
    level: float
    bz_pos_id: Optional[Any] = None  # PositionId type
    cl_pos_id: Optional[Any] = None  # PositionId type
    # 可以扩展记录：建仓价、建仓时间等


@dataclass
class PairedOrderTracker:
    """Track paired BZ+CL orders to detect partial fills"""
    level: float
    bz_order_id: Any
    cl_order_id: Any
    bz_filled: bool = False
    cl_filled: bool = False
    submit_time: int = 0  # timestamp in nanoseconds
    bz_leg: str = ""  # "BZ_LONG" or "BZ_SHORT"
    cl_leg: str = ""  # "CL_LONG" or "CL_SHORT"


@dataclass
class PairedCloseTracker:
    """Track paired BZ+CL close orders to detect partial closes"""
    level: float
    bz_order_id: Optional[Any] = None
    cl_order_id: Optional[Any] = None
    bz_filled: bool = False
    cl_filled: bool = False
    submit_time: int = 0  # timestamp in nanoseconds
...
class BzClGridStrategy(Strategy):
    def __init__(self, config: BzClGridConfig) -> None:
        super().__init__(config)

        # Instruments
        self.bz_id = InstrumentId.from_str(config.bz_instrument_id)
        self.cl_id = InstrumentId.from_str(config.cl_instrument_id)

        self.bz = None
        self.cl = None

        # 最新报价
        self.bz_bid: Optional[float] = None
        self.bz_ask: Optional[float] = None
        self.cl_bid: Optional[float] = None
        self.cl_ask: Optional[float] = None

        # 网格状态：level -> GridPositionState
        self.grid_state: Dict[float, GridPositionState] = {}

        # 在途订单追踪：order_id -> (level, leg)
        # leg: "BZ_LONG", "BZ_SHORT", "CL_LONG", "CL_SHORT"
    ...
    def on_start(self) -> None:
        self.log.info(
            f"BzClGridStrategy starting on Bybit, target_leverage={self.config.target_leverage}x, "
            f"max_total_notional={self.config.max_total_notional}."
        )

        # 获取 instrument
        self.bz = self.cache.instrument(self.bz_id)
        self.cl = self.cache.instrument(self.cl_id)

        if self.bz is None or self.cl is None:
            raise RuntimeError(
                f"Instruments not found in cache. Check instruments config: "
                f"bz={self.bz_id}, cl={self.cl_id}"
            )
        ...
        if self.config.auto_subscribe:
            ...
            self.subscribe_quote_ticks(instrument_id=self.bz_id)
            self.subscribe_quote_ticks(instrument_id=self.cl_id)
            self.log.info(
                f"Subscribed to quote ticks: BZ={self.bz_id}, CL={self.cl_id}"
            )
    ...
    def on_stop(self) -> None:
        self.log.info("BzClGridStrategy stopping, cancelling working orders and closing positions...")
        # 撤销所有挂单
    ...
    def _sync_existing_positions(self) -> None:
        ...
        if self.config.initial_notional_override > 0:
            ...
            for level in levels_reverse:
                ...
                if remaining_notional >= notional_for_level * 0.8:  # 80% threshold for matching
                    ...
                    state.bz_pos_id = "MANUAL_OVERRIDE"
                    state.cl_pos_id = "MANUAL_OVERRIDE"
        ...
        for pos in self.cache.positions_open():
            if pos.instrument_id == self.bz_id:
                bz_pos = pos
                bz_notional = abs(float(pos.quantity) * float(pos.avg_px_open))
                self.log.info(
                    f"[cache.positions_open] Found BZ position: qty={pos.quantity}, "
                    f"side={'LONG' if pos.is_long else 'SHORT'}, notional={bz_notional:.2f}"
                )
            elif pos.instrument_id == self.cl_id:
                cl_pos = pos
                cl_notional = abs(float(pos.quantity) * float(pos.avg_px_open))
                self.log.info(
                    f"[cache.positions_open] Found CL position: qty={pos.quantity}, "
                    f"side={'LONG' if pos.is_long else 'SHORT'}, notional={cl_notional:.2f}"
                )
        ...
        if bz_pos is None and cl_pos is None:
            try:
                # Get net exposure from portfolio (uses current prices)
                bz_exposure = self.portfolio.net_exposure(self.bz_id)
                cl_exposure = self.portfolio.net_exposure(self.cl_id)

                if bz_exposure is not None:
                    bz_notional = abs(float(bz_exposure))
                    if bz_notional > 0:
                        self.log.info(f"[portfolio.net_exposure] BZ exposure: {bz_notional:.2f}")

                if cl_exposure is not None:
                    cl_notional = abs(float(cl_exposure))
                    if cl_notional > 0:
                        self.log.info(f"[portfolio.net_exposure] CL exposure: {cl_notional:.2f}")

            except Exception as e:
                self.log.warning(f"Error checking portfolio.net_exposure: {e}")

        # Method 3: Also check cache.positions() as fallback
        if bz_pos is None and cl_pos is None and bz_notional == 0 and cl_notional == 0:
            for pos in self.cache.positions():
                if pos.instrument_id == self.bz_id and pos.is_open:
                    bz_pos = pos
                    bz_notional = abs(float(pos.quantity) * float(pos.avg_px_open))
                    self.log.info(f"[cache.positions] Found BZ: notional={bz_notional:.2f}")
                elif pos.instrument_id == self.cl_id and pos.is_open:
                    cl_pos = pos
                    cl_notional = abs(float(pos.quantity) * float(pos.avg_px_open))
                    self.log.info(f"[cache.positions] Found CL: notional={cl_notional:.2f}")

        # Calculate total and sync state
        total_detected = bz_notional + cl_notional
        ...
        if total_detected > 0:
            ...
            for level in levels_reverse:
                ...
                if remaining_notional >= notional_for_level * 0.8:  # 80% threshold for matching
                    ...
                    if bz_pos is not None:
                        state.bz_pos_id = bz_pos.id
                    else:
                        state.bz_pos_id = "DETECTED"  # Marker for detected but not tracked
                    if cl_pos is not None:
                        state.cl_pos_id = cl_pos.id
                    else:
                        state.cl_pos_id = "DETECTED"
            ...
            self.log.warning(
                ...
                f"total_notional={self.total_notional:.2f} (BZ={bz_notional:.2f}, CL={cl_notional:.2f}). "
    ...
    def on_quote_tick(self, tick: QuoteTick) -> None:
        # 更新最新报价
        if tick.instrument_id == self.bz_id:
            self.bz_bid = float(tick.bid_price)
            self.bz_ask = float(tick.ask_price)
        elif tick.instrument_id == self.cl_id:
            self.cl_bid = float(tick.bid_price)
            self.cl_ask = float(tick.ask_price)
    ...
    def _update_notional_from_portfolio(self) -> None:
        ...
        try:
            ...
            try:
                bz_exp = self.portfolio.net_exposure(self.bz_id)
                cl_exp = self.portfolio.net_exposure(self.cl_id)
                if bz_exp is not None:
                    bz_notional = abs(float(bz_exp))
                if cl_exp is not None:
                    cl_notional = abs(float(cl_exp))
            except Exception as e:
                self.log.warning(
                ...
                for pos in self.cache.positions_open():
                    if pos.instrument_id == self.bz_id:
                        bz_notional += float(pos.quantity) * float(pos.avg_px_open)
                    elif pos.instrument_id == self.cl_id:
                        cl_notional += float(pos.quantity) * float(pos.avg_px_open)

            new_total = bz_notional + cl_notional
            if abs(new_total - self.total_notional) > 1.0:  # Only log if significant change
                self.log.info(
                    ...
                    f"(BZ={bz_notional:.2f}, CL={cl_notional:.2f})"
                )
    ...
    def _handle_order_failure(self, order_id: Any, reason: str) -> None:
        ...
        for submit_time, tracker in list(self.paired_orders.items()):
            if tracker.bz_order_id == order_id:
                notional = self._get_level_notional(tracker.level)
                self.log.warning(f"BZ order {reason} for level={tracker.level}")
                # 如果 CL 已经成交，需要平掉
                if tracker.cl_filled:
                    self.log.warning(f"CL already filled, closing CL position for level={tracker.level}")
                    state = self.grid_state.get(tracker.level)
                    if state and state.cl_pos_id:
                        correction_order = self._close_position(state.cl_pos_id)
                        if correction_order is not None:
                            self._rebalance_order_ids.add(correction_order.client_order_id)
                        state.cl_pos_id = None
                else:
                    # CL 还没成交，取消它
                    self._safe_cancel_order(tracker.cl_order_id)
                # 清理pending_notional
                ...
                break
            elif tracker.cl_order_id == order_id:
                notional = self._get_level_notional(tracker.level)
                self.log.warning(f"CL order {reason} for level={tracker.level}")
                # 如果 BZ 已经成交，需要平掉
                if tracker.bz_filled:
                    self.log.warning(f"BZ already filled, closing BZ position for level={tracker.level}")
                    state = self.grid_state.get(tracker.level)
                    if state and state.bz_pos_id:
                        correction_order = self._close_position(state.bz_pos_id)
                        if correction_order is not None:
                            self._rebalance_order_ids.add(correction_order.client_order_id)
                        state.bz_pos_id = None
                else:
                    # BZ 还没成交，取消它
                    self._safe_cancel_order(tracker.bz_order_id)
                # 清理pending_notional
                self.pending_notional = max(0.0, self.pending_notional - 2 * notional)
                self.log.info(f"Order failure cleanup, pending_notional={self.pending_notional:.2f}")
                del self.paired_orders[submit_time]
    ...
    def on_order_filled(self, event) -> None:
        ...
        for submit_time, tracker in self.paired_orders.items():
            notional = self._get_level_notional(tracker.level)
            if tracker.bz_order_id == event.client_order_id:
                tracker.bz_filled = True
                tracker_submit_time = submit_time
                self.log.debug(f"BZ order filled for level={tracker.level}")
                # 检查是否两边都成交
                if tracker.cl_filled:
                    both_filled = True
                    # 从待确认转移到已确认
                ...
                break
            elif tracker.cl_order_id == event.client_order_id:
                tracker.cl_filled = True
                tracker_submit_time = submit_time
                self.log.debug(f"CL order filled for level={tracker.level}")
                # 检查是否两边都成交
                if tracker.bz_filled:
                    both_filled = True
                    # 从待确认转移到已确认
        ...
        if leg == "BZ_LONG":
            state.bz_pos_id = pos.id if pos is not None else "FILLED"
        elif leg == "BZ_SHORT":
            state.bz_pos_id = pos.id if pos is not None else "FILLED"
        elif leg == "CL_LONG":
            state.cl_pos_id = pos.id if pos is not None else "FILLED"
        elif leg == "CL_SHORT":
            state.cl_pos_id = pos.id if pos is not None else "FILLED"
        ...
        if both_filled and tracker_submit_time is not None:
            if state.bz_pos_id is not None and state.cl_pos_id is not None:
                del self.paired_orders[tracker_submit_time]
                self.log.debug(
                    f"Removed tracker for level={level} after both fills confirmed "
                    f"and positions set (BZ={state.bz_pos_id}, CL={state.cl_pos_id})"
                )
    ...
    def _handle_close_order_fill(self, event) -> None:
        ...
        for submit_time, tracker in list(self.paired_close_orders.items()):
            ...
            if tracker.bz_order_id == order_id:
                tracker.bz_filled = True
                self.log.info(f"BZ close order filled for level={tracker.level}")
                ...
                tracker.cl_filled = True
                self.log.info(f"CL close order filled for level={tracker.level}")
    ...
    def _grid_has_position(self, state: GridPositionState) -> bool:
        return (state.bz_pos_id is not None) or (state.cl_pos_id is not None)
    ...
    def _open_grid(self, level: float, spread: float) -> None:
        """
        spread > 0: BZ 贵 → 空 BZ，多 CL
        spread < 0: CL 贵 → 空 CL，多 BZ
        ...
        """
        if (self.bz_bid is None or self.bz_ask is None or
                self.cl_bid is None or self.cl_ask is None):
            return

        if spread > 0:
            # 空 BZ，多 CL
            bz_side = OrderSide.SELL
            cl_side = OrderSide.BUY
            bz_leg_tag = "BZ_SHORT"
            cl_leg_tag = "CL_LONG"
        else:
            # 空 CL，多 BZ
            bz_side = OrderSide.BUY
            cl_side = OrderSide.SELL
            bz_leg_tag = "BZ_LONG"
            cl_leg_tag = "CL_SHORT"

        # Use mid price for quantity calculation (market orders fill at best available)
        bz_mid = self._mid_price(self.bz_bid, self.bz_ask)
        cl_mid = self._mid_price(self.cl_bid, self.cl_ask)

        notional = self._get_level_notional(level)
        bz_qty = notional / bz_mid
        cl_qty = notional / cl_mid
        ...
        # BZ has lower liquidity than CL and the limit price rarely crossed.
        bz_order = self.order_factory.market(
            instrument_id=self.bz_id,
            order_side=bz_side,
            quantity=self.bz.make_qty(bz_qty),
            time_in_force=TimeInForce.IOC,
        )

        cl_order = self.order_factory.market(
            instrument_id=self.cl_id,
            order_side=cl_side,
            quantity=self.cl.make_qty(cl_qty),
            time_in_force=TimeInForce.IOC,
        )

        # 提交订单
        self.submit_order(bz_order)
        self.submit_order(cl_order)

        self.log.info(
            f"Submitted MARKET orders for grid level={level}: "
            f"{bz_leg_tag} qty={bz_qty:.6f} (mid={bz_mid:.4f}), "
            f"{cl_leg_tag} qty={cl_qty:.6f} (mid={cl_mid:.4f})"
        )

        # 记录在途订单
        self.working_orders[bz_order.client_order_id] = (level, bz_leg_tag)
        self.working_orders[cl_order.client_order_id] = (level, cl_leg_tag)

        # 创建配对订单追踪器
        submit_time = self.clock.timestamp_ns()
        tracker = PairedOrderTracker(
            level=level,
            bz_order_id=bz_order.client_order_id,
            cl_order_id=cl_order.client_order_id,
    ...
    def _close_grid(self, level: float, state: GridPositionState, force: bool = False) -> None:
        ...
        if state.bz_pos_id is None and state.cl_pos_id is None:
        ...
        if state.bz_pos_id is not None:
            bz_order = self._close_position(state.bz_pos_id, instrument_id=self.bz_id)
            if bz_order is None:
                self.log.warning(f"Failed to submit BZ close order for level={level}")

        if state.cl_pos_id is not None:
            cl_order = self._close_position(state.cl_pos_id, instrument_id=self.cl_id)
            if cl_order is None:
                self.log.warning(f"Failed to submit CL close order for level={level}")

        # If no orders were submitted, clear the state
        if bz_order is None and cl_order is None:
        ...
        tracker = PairedCloseTracker(
            level=level,
            bz_order_id=bz_order.client_order_id if bz_order else None,
            cl_order_id=cl_order.client_order_id if cl_order else None,
            submit_time=submit_time,
            bz_filled=bz_order is None,  # If no order, mark as "filled"
            cl_filled=cl_order is None,
        )
        self.paired_close_orders[submit_time] = tracker

        self.log.info(
            f"Submitted close orders for grid level={level}: "
            f"BZ={bz_order.client_order_id if bz_order else 'N/A'}, "
            f"CL={cl_order.client_order_id if cl_order else 'N/A'}"
        )
    ...
    def _rebalance_if_needed(self) -> None:
        """
        Corrects leg imbalance between BZ and CL positions.
        ...
        # portfolio look like a 100% imbalance (e.g. bz_notional=0, cl_notional=145).
        ...
        bz_side: Optional[OrderSide] = None
        cl_side: Optional[OrderSide] = None

        bz_price = self._mid_price(self.bz_bid, self.bz_ask)
        cl_price = self._mid_price(self.cl_bid, self.cl_ask)

        try:
            bz_exp_raw = self.portfolio.net_exposure(self.bz_id)
            cl_exp_raw = self.portfolio.net_exposure(self.cl_id)
            bz_exp_val = float(bz_exp_raw) if bz_exp_raw is not None else 0.0
            cl_exp_val = float(cl_exp_raw) if cl_exp_raw is not None else 0.0
        except Exception as e:
        ...
        if bz_exp_val == 0.0 and cl_exp_val == 0.0:
            self.log.debug(
            ...
            for pos in self.cache.positions_open():
                ...
                if pos.instrument_id == self.bz_id and bz_price is not None:
                    bz_exp_val += sign * float(pos.quantity) * bz_price
                elif pos.instrument_id == self.cl_id and cl_price is not None:
                    cl_exp_val += sign * float(pos.quantity) * cl_price

        bz_notional = abs(bz_exp_val)
        cl_notional = abs(cl_exp_val)

        # Infer direction from exposure sign
        if bz_exp_val > 0:
            bz_side = OrderSide.BUY   # long BZ
        elif bz_exp_val < 0:
            bz_side = OrderSide.SELL  # short BZ
        if cl_exp_val > 0:
            cl_side = OrderSide.BUY   # long CL
        elif cl_exp_val < 0:
            cl_side = OrderSide.SELL  # short CL

        if bz_notional == 0.0 and cl_notional == 0.0:
            return

        delta = bz_notional - cl_notional
        base = max(abs(bz_notional), abs(cl_notional), 1.0)
        ...
        self.log.info(
            f"Rebalancing, bz_notional={bz_notional:.2f}, "
            f"cl_notional={cl_notional:.2f}, imbalance={imbalance:.4%}"
        )
        ...
        if bz_notional > cl_notional:
            # CL is the lagging leg: add to CL in the same direction as existing CL.
            # Infer direction from BZ side when no CL position exists yet
            # (spread > 0 → SHORT BZ / LONG CL → CL correction is BUY).
            if cl_side is not None:
                correction_side = cl_side
            elif bz_side == OrderSide.SELL:
                correction_side = OrderSide.BUY   # BZ short → CL should be long
            else:
                correction_side = OrderSide.SELL  # BZ long → CL should be short
            if cl_price is None:
                return
            qty = correction_notional / cl_price
            # Pre-check BEFORE make_qty: it raises ValueError when qty rounds to zero
            if qty < float(self.cl.size_increment):
                self.log.info(
                    f"Rebalance correction too small ({correction_notional:.2f} USDT "
                    f"→ {qty:.8f} CL < min {float(self.cl.size_increment):.4f}), skipping"
                )
            ...
            order = self.order_factory.market(
                ...
                quantity=self.cl.make_qty(qty),
            )
            leg_name = "CL"
        else:
            # BZ is the lagging leg: add to BZ in the same direction as existing BZ.
            if bz_side is not None:
                correction_side = bz_side
            elif cl_side == OrderSide.BUY:
                correction_side = OrderSide.SELL  # CL long → BZ should be short
            else:
                correction_side = OrderSide.BUY   # CL short → BZ should be long
            if bz_price is None:
                return
            qty = correction_notional / bz_price
            # Pre-check BEFORE make_qty: it raises ValueError when qty rounds to zero
            if qty < float(self.bz.size_increment):
                self.log.info(
                    f"Rebalance correction too small ({correction_notional:.2f} USDT "
                    f"→ {qty:.8f} BZ < min {float(self.bz.size_increment):.4f}), skipping"
                )
                self._last_rebalance_ns = current_time
                return
            order = self.order_factory.market(
        ...
        # This catches cases where qty > size_increment but value < 5 USDT (e.g. 0.001 CL
        # at $2900 = $2.90 which passes the size_increment check but is rejected by Bybit).
        ref_price = cl_price if leg_name == "CL" else bz_price
    ...
    def _reconcile_positions(self) -> None:
        ...
        actual_bz_notional = self._get_actual_position_notional(self.bz_id)
        actual_cl_notional = self._get_actual_position_notional(self.cl_id)
        actual_total = actual_bz_notional + actual_cl_notional
        ...
        self.log.info(
            ...
            f"actual={actual_total:.2f} (BZ={actual_bz_notional:.2f}, CL={actual_cl_notional:.2f}), "
            f"diff={diff:.2f}"
        )
        ...
        # A stable total with one oversized leg (e.g. BZ=600, CL=300) would never
        # be caught by the diff > 100 check above, so we test independently here.
        if actual_total > 0:
            imbalance = abs(actual_bz_notional - actual_cl_notional) / actual_total
            if imbalance > 0.20:  # 20% imbalance
                ...
                if self.paired_close_orders:
                    self.log.warning(
                        ...
                        f"(BZ={actual_bz_notional:.2f}, CL={actual_cl_notional:.2f}) "
    ...
    def _get_actual_position_notional(self, instrument_id: InstrumentId) -> float:
        ...
        for pos in self.cache.positions_open():
            if pos.instrument_id == instrument_id:
                ...
                if instrument_id == self.bz_id:
                    price = self._mid_price(self.bz_bid, self.bz_ask)
                else:
                    price = self._mid_price(self.cl_bid, self.cl_ask)
    ...
    def _has_valid_quotes(self) -> bool:
        return all([
            self.bz_bid is not None,
            self.bz_ask is not None,
            self.cl_bid is not None,
            self.cl_ask is not None,
        ])
    ...
    def _calc_spread(self) -> Optional[float]:
        ...
        bz_mid = self._mid_price(self.bz_bid, self.bz_ask)
        cl_mid = self._mid_price(self.cl_bid, self.cl_ask)
        if bz_mid is None or cl_mid is None:
            return None
        return (bz_mid - cl_mid) / cl_mid
    ...
    def _get_bid_ask(self, inst: InstrumentId) -> Tuple[Optional[float], Optional[float]]:
        if inst == self.bz_id:
            return self.bz_bid, self.bz_ask
        else:
            return self.cl_bid, self.cl_ask
    ...
    def _check_order_timeouts(self) -> None:
        ...
        - After order_timeout_sec seconds have elapsed since submission,
          inspects each PairedOrderTracker for fill status:
            ...
            BZ filled, CL not filled:
                Cancel CL limit order.
                Close BZ position via market IOC (restore market neutrality).

            CL filled, BZ not filled:
                Cancel BZ limit order.
                Close CL position via market IOC (restore market neutrality).
        ...
        for submit_time, tracker in list(self.paired_orders.items()):
            ...
            # 检查是否两边都成交了
            if tracker.bz_filled and tracker.cl_filled:
                # 两边都成交，正常情况，清理追踪器
                del self.paired_orders[submit_time]
            ...
            if tracker.bz_filled and not tracker.cl_filled:
                self.log.warning(
                    f"IMBALANCED FILL DETECTED: BZ filled but CL not filled for level={tracker.level}. "
                    f"Canceling CL order and closing BZ position to prevent directional exposure."
                )
                # 取消未成交的 CL 订单
                self._safe_cancel_order(tracker.cl_order_id)
                # 平掉已成交的 BZ 仓位
                ...
                if state and state.bz_pos_id:
                    ...
                    if correction_order is not None:
                        ...
                        self._rebalance_order_ids.add(correction_order.client_order_id)
                    state.bz_pos_id = None
                # BZ成交了，但配对失败，需要从pending中扣除全部（因为CL没成交，不会进入total）
                # 注意：BZ成交时已经在on_order_filled中等待配对，这里只需要清理pending
                self.pending_notional = max(0.0, self.pending_notional - 2 * notional)
                self.log.info(f"Cleaned up imbalanced pair, pending_notional={self.pending_notional:.2f}")
                # 清理追踪器
                del self.paired_orders[submit_time]

            elif tracker.cl_filled and not tracker.bz_filled:
                self.log.warning(
                    f"IMBALANCED FILL DETECTED: CL filled but BZ not filled for level={tracker.level}. "
                    f"Canceling BZ order and closing CL position to prevent directional exposure."
                )
                # 取消未成交的 BZ 订单
                self._safe_cancel_order(tracker.bz_order_id)
                # 平掉已成交的 CL 仓位
                ...
                if state and state.cl_pos_id:
                    ...
                    if correction_order is not None:
                        self._rebalance_order_ids.add(correction_order.client_order_id)
                    state.cl_pos_id = None
                # CL成交了，但配对失败，需要从pending中扣除全部
                self.pending_notional = max(0.0, self.pending_notional - 2 * notional)
                self.log.info(f"Cleaned up imbalanced pair, pending_notional={self.pending_notional:.2f}")
                # 清理追踪器
                del self.paired_orders[submit_time]

            elif not tracker.bz_filled and not tracker.cl_filled:
    ...
    def _check_close_order_timeouts(self) -> None:
        ...
        for submit_time, tracker in list(self.paired_close_orders.items()):
            ...
            if tracker.bz_filled and tracker.cl_filled:
            ...
            elif tracker.bz_filled and not tracker.cl_filled:
                # BZ closed but CL didn't - CRITICAL imbalance!
                self.log.error(
                    f"🚨 IMBALANCED CLOSE: BZ closed but CL still open at level={tracker.level}! "
                    f"Will retry closing CL."
                )
                # Cancel old order and retry
                if tracker.cl_order_id:
                    self._safe_cancel_order(tracker.cl_order_id)
                # Retry closing CL
                state = self.grid_state.get(tracker.level)
                if state and state.cl_pos_id:
                    new_order = self._close_position(state.cl_pos_id)
                    if new_order:
                        # Update tracker with new order
                        tracker.cl_order_id = new_order.client_order_id
                        tracker.submit_time = current_time
                        self.log.info(f"Resubmitted CL close order: {new_order.client_order_id}")
                else:
                ...
                # CL closed but BZ didn't - CRITICAL imbalance!
                self.log.error(
                    f"🚨 IMBALANCED CLOSE: CL closed but BZ still open at level={tracker.level}! "
                    f"Will retry closing BZ."
                )
                # Cancel old order and retry
                if tracker.bz_order_id:
                    self._safe_cancel_order(tracker.bz_order_id)
                # Retry closing BZ
                state = self.grid_state.get(tracker.level)
                if state and state.bz_pos_id:
                    new_order = self._close_position(state.bz_pos_id)
                    if new_order:
                        # Update tracker with new order
                        tracker.bz_order_id = new_order.client_order_id
                        tracker.submit_time = current_time
                        self.log.info(f"Resubmitted BZ close order: {new_order.client_order_id}")
                else:
                    # Position already gone? Clear state
                    if state:
                        state.bz_pos_id = None
    ...
    def _estimate_net_pnl(self, level: float, state: GridPositionState) -> Optional[float]:
        ...
        # BZ leg
        if state.bz_pos_id is not None:
            pos = self._find_position(state.bz_pos_id, self.bz_id)
            if pos is None:
                self.log.debug(f"_estimate_net_pnl: BZ position not found for level={level}")
                return None
            entry = float(pos.avg_px_open)
            qty   = float(pos.quantity)
            gross_pnl    += (entry - bz_mid) * qty if pos.is_short else (bz_mid - entry) * qty
            open_notional += entry * qty

        # CL leg
        if state.cl_pos_id is not None:
            pos = self._find_position(state.cl_pos_id, self.cl_id)
            if pos is None:
                self.log.debug(f"_estimate_net_pnl: CL position not found for level={level}")
                return None
            entry = float(pos.avg_px_open)
            qty   = float(pos.quantity)
            gross_pnl    += (entry - cl_mid) * qty if pos.is_short else (cl_mid - entry) * qty
            open_notional += entry * qty
...
def create_strategy(config: BzClGridConfig) -> BzClGridStrategy:
    return BzClGridStrategy(config=config)
